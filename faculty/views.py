...
def write_application_faculty(request):
    if request.method=='POST':
        ...
        appln.is_published_by_faculty=True
    ...
    return render(request,'faculty/create-application.html',context)
def faculty_application_details(request,application_no):
    auth=None
    if request.user.is_authenticated:
        auth=UserProfile.objects.get(user_id=request.user.id)
    userprofile=UserProfile.objects.get(user_id=request.user.id)
    ...
    return render(request,'faculty/view-application.html',context)
